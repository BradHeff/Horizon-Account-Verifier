...
class Updater():
    ...
    def writeLines(self):
        ...
        with open(paths + "\\version.rc", "w") as w:
            ...
            self.vlines[pos3] = self.vlines[pos3].replace(
                self.FileVersion, self.new)
            self.vlines[pos4] = self.vlines[pos4].replace(
                self.ProductVersion, self.new)
