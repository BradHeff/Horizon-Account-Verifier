...
class Updater():
    ...
    def readLines(self):
        ...
        with open(paths + "\\version.rc", "r") as f:
            ...
            self.prodvers = line2.split("=")[1].replace('(', '').replace(')', '').strip()
            self.prodvers = self.prodvers[0:self.prodvers.__len__()-1]

            self.FileVersion = line3.split(",")[1].replace('\'', '').replace(')', '').replace('u', '').strip()
            self.FileVersion = self.FileVersion[0:self.FileVersion.__len__()]

            self.ProductVersion = line4.split(",")[1].replace('\'', '').replace(')', '').replace('u', '').strip()
    ...
    def writeLines(self):
        with open(paths + "\\Functions.py", "w") as w:
            ...
            self.lines[pos] = self.lines[pos].replace(self.old, ''.join(["v",self.new]))
        ...
        with open(paths + "\\version.rc", "w") as w:
            ...
            self.vlines[pos1] = self.vlines[pos1].replace(self.filevers, self.new.replace('.', ','))
            self.vlines[pos2] = self.vlines[pos2].replace(self.prodvers, self.new.replace('.', ','))
